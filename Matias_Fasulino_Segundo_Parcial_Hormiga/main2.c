...
int main(int argc, char* argv[]) {
	...
	*memoria += 1;
	...
	datos_animal = (Animal*)malloc(sizeof(Animal)*cantidad);
		...
		strcpy(datos_animal[0].nombre, "Reina");
		datos_animal[0].id_cola_msg = id_cola_mensaje;
		datos_animal[0].velocidad_min = REINA_VEL_MIN;
		datos_animal[0].velocidad_max = REINA_VEL_MAX;
		pthread_create(&idHilo[0], &atributos, threadAnimal, &datos_animal[0]);
		/* Lanzo Hormiga Roja */
		datos_animal[1].id_animal = 1;
		strcpy(datos_animal[1].nombre, "Roja");
		datos_animal[1].id_cola_msg = id_cola_mensaje;
		datos_animal[1].velocidad_min = ROJA_VEL_MIN;
		datos_animal[1].velocidad_max = ROJA_VEL_MAX;
		pthread_create(&idHilo[1], &atributos, threadAnimal, &datos_animal[1]);
		/* Lanzo Hormiga Negra */
		datos_animal[2].id_animal = 2;
		strcpy(datos_animal[2].nombre, "Negra");
		datos_animal[2].id_cola_msg = id_cola_mensaje;
		datos_animal[2].velocidad_min = NEGRA_VEL_MIN;
		datos_animal[2].velocidad_max = NEGRA_VEL_MAX;
		pthread_create(&idHilo[2], &atributos, threadAnimal, &datos_animal[2]);

	for(i=0; i<cantidad; i++) {
		pthread_join(idHilo[i], NULL);
		/*printf("La hormiga %s llego a la meta.\n", datos_animal[i].nombre);*/
